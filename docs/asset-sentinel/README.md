...
| `-Fix` | **Repair the auto-fixable issues, then re-scan to prove it.** The fix pass's own success count is only a claim; the exit code trusts the re-scan. Issues it repaired are marked `[FIXED]` in the report. |
| `-DryRun` | Only meaningful with `-Fix`. Reports how many issues *would* be repaired and changes nothing. Run this first. |

### Repairing, not just reporting

Asset Sentinel does not only tell you what is wrong. `-Fix` applies every repair the active rule set
marks auto-fixable, and then **scans the project again** — so the pass/fail you get back is measured
against the project as it now stands, not against the repair code's opinion of itself. Anything it
could not repair is logged as `Fix failed: <reason>` and still counts against `-FailOn`.

```
REM see what would change, touch nothing
UnrealEditor-Cmd.exe "YourProject.uproject" -run=AssetSentinel -Fix -DryRun -unattended -nopause

REM repair, then verify
UnrealEditor-Cmd.exe "YourProject.uproject" -run=AssetSentinel -Fix -unattended -nopause
```

Source control first: `-Fix` writes to your assets.
